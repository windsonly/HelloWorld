...
public class QQService {
		...
		if (MSG_ACK_ENABLED) { // 如果启用了消息送达确认
			...
			xiaoVListener = new SmartQQClient(new MessageCallback() {
				...
				public void onGroupMessage(final GroupMessage message) {
					...
					if (GROUP_SENT_MSGS.contains(content)) { // indicates
																// message
																// received
				...
				public void onDiscussMessage(final DiscussMessage message) {
					...
					if (DISCUSS_SENT_MSGS.contains(content)) { // indicates
																// message
																// received
	...
	private void sendToForum(final String msg, final String user) {
		...
		try {
			...
			final String body = "key=" + URLEncoder.encode(forumKey, "UTF-8") + "&msg="
					+ URLEncoder.encode(msg, "UTF-8") + "&user=" + URLEncoder.encode(user, "UTF-8");
		...
		try {
			...
			for (final Map.Entry<Long, Group> entry : QQ_GROUPS.entrySet()) {
				...
				if (Strings.contains(name, groups)) {
					...
					xiaoV.sendMessageToGroup(group.getId(), msg); // Without
																	// retry
	...
	private void sendMessageToGroup(final Long groupId, final String msg) {
		...
		if (null == group) {
			LOGGER.log(Level.ERROR,
					"Group list error [groupId=" + groupId + "], 请先参考项目主页 FAQ 解决"
	...
	private void sendMessageToDiscuss(final Long discussId, final String msg) {
		...
		if (null == discuss) {
			LOGGER.log(Level.ERROR,
					"Discuss list error [discussId=" + discussId + "], 请先参考项目主页 FAQ 解决"
							+ "（https://github.com/b3log/xiaov#报错-group-list-error-groupidxxxx-please-report-this-bug-to-developer-怎么破），"
							+ "如果还有问题，请到论坛讨论帖中进行反馈（https://hacpai.com/article/1467011936362）");
	...
	public void onQQGroupMessage(final GroupMessage message) {
		...
		if (StringUtils.contains(content, XiaoVs.QQ_BOT_NAME)
				|| (StringUtils.length(content) > 6 && (StringUtils.contains(content, "?")
						|| StringUtils.contains(content, "？") || StringUtils.contains(content, "问")))) {
	...
	private String skipBotName(String content) {
		content = content.trim();
		if (content.startsWith(XiaoVs.QQ_BOT_NAME)) {
			content = content.replace(XiaoVs.QQ_BOT_NAME, "");
			return skipBotName(content);
		}
		if (content.startsWith(XiaoVs.QQ_BOT_NAME + " ")) {
			content = content.replace(XiaoVs.QQ_BOT_NAME + " ", "");
			return skipBotName(content);
	...
	private String answer(final String content, final String userName) {
		...
		for (int i = 0; i < keywords.length; i++) {
			if (StringUtils.containsIgnoreCase(search, keywords[i])) {
				keyword = keywords[i];
				keywordIndex = i;
				break;
			}
		...
		if (StringUtils.isNotBlank(keyword)) {
			try {
				String[] searchEngines = StringUtils.split(XiaoVs.getString("bot.follow.keywordAnswer"), ";");
				search = search.replaceFirst("^.*" + keyword, "");
				ret = StringUtils.replace(searchEngines[keywordIndex], "{keyword}", URLEncoder.encode(search, "UTF-8"));
